...
public class GuiDialogInteract extends GuiNPCInterface
{
    ...
    private void drawWheel(){
        ...
        for(int slot:dialog.options.keySet()){
        	...
        	if(slot == 3)
        		drawString(fontRendererObj, option.title, width/2 - 13 - fontRendererObj.getStringWidth(option.title),yoffset - 6  , color);
        	if(slot == 4)
        		drawString(fontRendererObj, option.title, width/2 - 33 - fontRendererObj.getStringWidth(option.title),yoffset + 12  , color);
        	if(slot == 5)
        		drawString(fontRendererObj, option.title, width/2 - 27 - fontRendererObj.getStringWidth(option.title),yoffset + 32  , color);
    ...
    private void drawLinedOptions(int j){
        ...
        if(j >= (int)((guiTop + offset))){
        	int selected = (int) ((j - (guiTop + offset)) / (fontRendererObj.FONT_HEIGHT));
        ...
        for(int k = 0; k < options.size(); k++){
        	...
        	int y = (int)((guiTop + offset  + (count * fontRendererObj.FONT_HEIGHT)));
        	if(selected == k){
