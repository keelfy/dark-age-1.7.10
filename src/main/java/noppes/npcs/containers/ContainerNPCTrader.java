...
public class ContainerNPCTrader extends ContainerNpcInterface {
	...
	public ContainerNPCTrader(EntityNPCInterface npc, EntityPlayer player) {
		...
		for (int i = 0; i < 18; i++) {
			...
			ItemStack item3 = role.inventoryCurrency.items.get(i + 36);
		...
		if(!player.capabilities.isCreativeMode) {
			PlayerContainerUtils.addPlayerInventorySlots(this, player.inventory, 31, 140);
		} else {
	...
	public ItemStack slotClick(int i, int j, int par3, EntityPlayer entityplayer) {
		...
		if (!canBuy(i, entityplayer))
			return null;
		NoppesUtilPlayer.consumeItem(entityplayer, role.inventoryCurrency.getStackInSlot(i), role.ignoreDamage,
				role.ignoreNBT);
		NoppesUtilPlayer.consumeItem(entityplayer, role.inventoryCurrency.getStackInSlot(i + 18), role.ignoreDamage,
				role.ignoreNBT);
		NoppesUtilPlayer.consumeItem(entityplayer, role.inventoryCurrency.getStackInSlot(i + 36), role.ignoreDamage,
				role.ignoreNBT);
	...
	public boolean canBuy(int slot, EntityPlayer player) {
		...
		ItemStack currency3 = role.inventoryCurrency.getStackInSlot(slot + 36);
		...
		if (NoppesUtilPlayer.compareItems(currency2, currency3, role.ignoreDamage, role.ignoreNBT)) {
			currency2 = currency2.copy();
			currency2.stackSize += currency3.stackSize;
			currency3 = null;
		}
		...
		if (currency2 != null && currency3 == null && currency != null)
			return NoppesUtilPlayer.compareItems(player, currency, role.ignoreDamage, role.ignoreNBT)
					&& NoppesUtilPlayer.compareItems(player, currency2, role.ignoreDamage, role.ignoreNBT);

		return NoppesUtilPlayer.compareItems(player, currency, role.ignoreDamage, role.ignoreNBT)
				&& NoppesUtilPlayer.compareItems(player, currency2, role.ignoreDamage, role.ignoreNBT)
				&& NoppesUtilPlayer.compareItems(player, currency3, role.ignoreDamage, role.ignoreNBT);
