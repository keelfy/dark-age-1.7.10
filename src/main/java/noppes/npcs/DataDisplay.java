...
public class DataDisplay {
	...
	public DataDisplay(final EntityNPCInterface npc) {
		...
		String[] names = { "Noppes", "Noppes", "Noppes", "Noppes", "Atesson", "Rothcersul", "Achdranys", "Pegato",
				...
				"Chris", "Christine", "Marble", "Mable", "Ross", "Rose", "Xalgan ", "Kennet" };
		...
		if (this.playerProfile != null && !StringUtils.isNullOrEmpty(this.playerProfile.getName())
			...
			if (!this.playerProfile.isComplete() || !this.playerProfile.getProperties().containsKey("textures")) {
				GameProfile gameprofile = MinecraftServer.getServer().func_152358_ax()
						.func_152655_a(this.playerProfile.getName());

				if (gameprofile != null) {
					Property property;
					Collection<Property> col = gameprofile.getProperties().get("textures");

					if (gameprofile.getProperties().get("textures").isEmpty()) {
						property = null;
					} else {
						property = (Property) col.toArray()[0];
					}

					// property = (Property)
					// Iterables.getFirst(gameprofile.getProperties().get("textures"),
					// (Object) null);
