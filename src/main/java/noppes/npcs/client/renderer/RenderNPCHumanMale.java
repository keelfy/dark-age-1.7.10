...
public class RenderNPCHumanMale extends RenderNPCInterface {
	...
	protected void renderSpecials(final EntityNPCInterface npc, final float f) {
		...
		if (!npc.display.cloakTexture.isEmpty()) {
			...
			double d = (npc.field_20066_r + (npc.field_20063_u - npc.field_20066_r) * f) - (npc.prevPosX + (npc.posX - npc.prevPosX) * f);
			double d1 = (npc.field_20065_s + (npc.field_20062_v - npc.field_20065_s) * f) - (npc.prevPosY + (npc.posY - npc.prevPosY) * f);
			double d2 = (npc.field_20064_t + (npc.field_20061_w - npc.field_20064_t) * f) - (npc.prevPosZ + (npc.posZ - npc.prevPosZ) * f);
		...
		DARenderArmor.renderAllArmor(npc, modelBipedMain);
	}
