...
public class GuiNpcTextArea extends GuiNpcTextField{
    ...
    public void mouseClicked(int i, int j, int k){
    	...
    	int x = i - posX;
    	int y = (j - posY - 4) / fontrenderer.FONT_HEIGHT + getStartLineY();
    	cursorPosition = 0;
        ...
        for(int g = 0; g < lines.size(); g++){
    		...
    		for(char c : wholeLine.toCharArray()){
        		...
        		if(fontrenderer.getStringWidth(line + c) > maxSize && wrapLine){
        			lineCount++;
        			line = "";
                    if(y < lineCount){
                    	break;
        		...
        		if(lineCount == y && x <= fontrenderer.getStringWidth(line + c))
            		return;
        		charCount++;
        		line += c;
	...
	private int getStartLineY(){
		if(!isScrolling())
			scrolledY = 0;
		return MathHelper.ceiling_double_int(scrolledY * (listHeight) / fontrenderer.FONT_HEIGHT);
	}
	...
	public void drawTextBox(int mouseX, int mouseY) {
        ...
        int maxLine = height / fontrenderer.FONT_HEIGHT + startLine;
    		...
    		for(char c : wholeLine.toCharArray()){
        		if(fontrenderer.getStringWidth(line + c) > maxSize && wrapLine){
        			if(lineCount >= startLine && lineCount < maxLine)
        				drawString(fontrenderer, line, posX + 4, posY + 4 + ((lineCount - startLine) * fontrenderer.FONT_HEIGHT), color);
        			line = "";
        			lineCount++;
        		}
            	if(flag && charCount == cursorPosition && lineCount >= startLine && lineCount < maxLine && canEdit){
        			int xx = posX + fontrenderer.getStringWidth(line) + 3;
        			int yy = posY + ((lineCount - startLine) * fontrenderer.FONT_HEIGHT) + 4;
            		if(getText().length() == cursorPosition){
                    	fontrenderer.drawString("_", xx, yy, color);  
            		}
            		else{
            			drawCursorVertical(xx, yy, xx + 1, yy + fontrenderer.FONT_HEIGHT);
            		}
            	}
        		charCount++;
        		line += c;
        ...
        listHeight = lineCount * fontrenderer.FONT_HEIGHT;
