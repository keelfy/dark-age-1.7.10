		...
		for (int slot = 0; slot < 18; slot++) {
			...
			if (this.func_146978_c(x + 43, y + 1, 16, 16, par1, par2)) {
				if (!container.canBuy(slot, player)) {
					...
					if (item2 != null
							&& !NoppesUtilPlayer.compareItems(player, item2, role.ignoreDamage, role.ignoreNBT))
						this.drawGradientRect(x - 1, y, x + 17, y + 18, 0x70771010, 0x70771010);
					if (item3 != null
							&& !NoppesUtilPlayer.compareItems(player, item3, role.ignoreDamage, role.ignoreNBT))
					...
					this.fontRendererObj.drawString(title, (xSize - fontRendererObj.getStringWidth(title)) / 2, 131,
							0xDD0000);
					GL11.glTranslatef(0, 0, -300);
				} else {
