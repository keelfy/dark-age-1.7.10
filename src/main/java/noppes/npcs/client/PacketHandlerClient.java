...
public class PacketHandlerClient extends PacketHandlerServer {
	...
	public void onPacketData(final ClientCustomPacketEvent event) {
		...
		try {
			client(buffer, player, EnumPacketClient.values()[buffer.readInt()]);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}

	private void client(final ByteBuf buffer, final EntityPlayer player, final EnumPacketClient type)
			throws IOException {
