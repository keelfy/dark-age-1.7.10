...
public class Dialog implements ICompatibilty {
	private int version = VersionCompatibility.ModRev;

	public int id = -1;
	public String title = KString.EMPTY;
	...
	public String sound;
	public String command = KString.EMPTY;
	...
	public boolean hasDialogs(final EntityPlayer player) {
		for (DialogOption option : options.values())
			if (option != null && option.optionType == EnumOptionType.DialogOption && option.hasDialog()
	...
	public Dialog copy(final EntityPlayer player) {
		...
		for (int slot : options.keySet()) {
			...
			if (option.optionType == EnumOptionType.DialogOption
					&& (!option.hasDialog() || !option.isAvailable(player))) {
				continue;
			}
