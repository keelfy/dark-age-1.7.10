...
public final class DialogController {
	...
	private int loadedDialogsCount = 0;
	...
	public void load() {
		...
		loadCategories();
		LogWriter.info("Loaded " + loadedDialogsCount + " Dialogs");
	}
	...
	private void loadCategories() {
		...
		try {
			File file = new File(CustomNpcs.getSaveDirectory(true), "dialog.dat");
			if (file.exists()) {
				loadCategoriesOld(file);
				file.delete();
				file = new File(CustomNpcs.getSaveDirectory(true), "dialog.dat_old");
				if (file.exists()) {
					file.delete();
				}
				return;
	...
	private File getDir() {
		return new File(CustomNpcs.getSaveDirectory(true), "dialogs");
	}
