...
public class GuiDialogInteract extends GuiNPCInterface {
	...
	public void drawScreen(final int i, final int j, final float f) {
		...
		if (!dialog.hideNPC) {
			float l = (guiLeft - 60);
			float i1 = (guiTop + 50 + dialogHeight);
		...
		for (TextBlockClient block : lines) {
			int size = fontRendererObj.getStringWidth(block.getName() + ": ");
	...
	private void drawWheel() {
		...
		for (int slot : dialog.options.keySet()) {
			...
			// drawString(fontRenderer, option.title, width/2 -50 ,yoffset+ 162 + slot * 13
			// , color);
			if (slot == 0) {
			...
			if (slot == 3) {
				drawString(fontRendererObj, option.title, width / 2 - 13 - fontRendererObj.getStringWidth(option.title),
						yoffset - 6, color);
			}
			if (slot == 4) {
				drawString(fontRendererObj, option.title, width / 2 - 33 - fontRendererObj.getStringWidth(option.title),
						yoffset + 12, color);
			}
			if (slot == 5) {
				drawString(fontRendererObj, option.title, width / 2 - 27 - fontRendererObj.getStringWidth(option.title),
						yoffset + 32, color);
			}
	...
	private void drawLinedOptions(final int j) {
		...
		if (j >= ((guiTop + offset))) {
			int selected = (j - (guiTop + offset)) / (fontRendererObj.FONT_HEIGHT);
		...
		for (int k = 0; k < options.size(); k++) {
			...
			int y = ((guiTop + offset + (count * fontRendererObj.FONT_HEIGHT)));
			if (selected == k) {
				drawString(fontRendererObj, ">", guiLeft - 60, y, 0xe0e0e0);
			}
			drawString(fontRendererObj, NoppesStringUtils.formatText(option.title, player, npc), guiLeft - 30, y,
					option.optionColor);
			count++;
		}
	}
	...
	private void drawString(final String text, final int left, final int color, final int count) {
		...
		super.drawString(fontRendererObj, text, guiLeft + left, guiTop + (height * fontRendererObj.FONT_HEIGHT), color);
	}
	...
	private void handleDialogSelection() {
		...
		if (option == null || option.optionType == EnumOptionType.QuitOption
	...
	private void calculateRowHeight() {
		...
		int max = dialogHeight / Minecraft.getMinecraft().fontRenderer.FONT_HEIGHT;
