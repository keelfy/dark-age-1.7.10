...
import keelfy.darkdata.client.gui.tabbed.InventoryTabFactions;
...
public class ClientProxy extends CommonProxy {
	...
	public void load() {
		...
		RenderingRegistry.registerEntityRenderingHandler(EntityNPCGolem.class,
				new RenderNPCHumanMale(new ModelNPCGolem(0), new ModelNPCGolem(1F), new ModelNPCGolem(0.5F)));
	...
	private GuiScreen getGui(final EntityNPCInterface npc, final EnumGuiType gui, final Container container,
		...
		else if (gui == EnumGuiType.PlayerBankSmall || gui == EnumGuiType.PlayerBankUnlock
		...
		if (string.equals("Spell")) {
			...
			for (int i = 0; i < number; i++) {
				...
				Minecraft.getMinecraft().effectRenderer
						.addEffect(new EntityElementalStaffFX(player, x, y, z, f, f1, f2, color));
			}
			...
			if (particles.type == 0) {
				for (int i = 0; i < 2; i++) {
					EntityEnderFX fx = new EntityEnderFX(npc, (rand.nextDouble() - 0.5D) * player.width,
							(rand.nextDouble() * player.height) - height - 0.25D,
							(rand.nextDouble() - 0.5D) * player.width, (rand.nextDouble() - 0.5D) * 2D,
							-rand.nextDouble(), (rand.nextDouble() - 0.5D) * 2D, particles);
